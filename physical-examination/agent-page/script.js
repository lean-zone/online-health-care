(() => {
  // ===== AI 问答接口配置（统一在项目根目录 llm-config.js，勿在此重复配置）=====
  const _LLM = window.LLM_CONFIG || {};
  const API_URL = _LLM.apiUrl;
  const API_KEY = _LLM.apiKey;
  const MODEL_NAME = _LLM.model;
...
· 需要用户从多个答案中选择时，必须调用 ask_question 工具，不要在正文里输出选项列表。题干填入 question 参数，各选项填入 options 参数（每项一个字符串、不带编号），多选时把 multi_select 设为 true、题干不要带"多选/多选题"标注。
· 当信息收集完成或用户表示"生成计划书/差不多了/就这些"时，调用一次 ask_question 工具并把 generate_plan 设为 true（question、options 留空）即可结束对话——这是结束对话的唯一方式，不要在正文输出【生成计划书】或其他承诺文字，也不要在对话中直接生成计划书正文或做总结/梳理/承诺/询问"您看行吗"等。
...
· 调用 ask_question 时，题干填在 question，例如：question="关于体检结果，你最想了解什么？"，options=["了解病情","寻求建议","学习相关知识"]。
· 禁止用「或者」「还是」等词语把选项直接写在正文里。
...
· 严禁在正文中使用「1. 2. 3.」阿拉伯数字编号列表（含 markdown 有序列表），需要列要点时用「① ② ③」「◆ ● ▪」或纯段落表达。
...
· 需要家长选择时，必须调用 ask_question 工具，把题干填入 question、各选项填入 options（每项一个字符串、不带编号），多选时把 multi_select 设为 true；禁止用「或者」「还是」把选项混写进正文
· 信息收集完成或家长表示"生成计划书/差不多了"时，调用一次 ask_question 工具并把 generate_plan 设为 true（question、options 留空）即可结束对话，不要在正文输出【生成计划书】或做总结/梳理/承诺
...
· 需要老人选择时，必须调用 ask_question 工具，把题干填入 question、各选项填入 options（每项一个字符串、不带编号），多选时把 multi_select 设为 true；禁止用「或者」「还是」把选项混写进正文
· 信息收集完成或老人表示"生成计划书/差不多了"时，调用一次 ask_question 工具并把 generate_plan 设为 true（question、options 留空）即可结束对话，不要在正文输出【生成计划书】或做总结/梳理/承诺
...
· 所有向用户的提问都必须通过调用 ask_question 工具提交：题干填 question、各选项填 options（每项一个字符串、不带编号），多选时把 multi_select 设为 true；不要在正文里输出选项列表或选项提示文字
· 可多选题的最后一个选项保留"以上都不是/不确定"类兜底项（放入 options 中，前端会自动处理与其他选项互斥）
· 提纲信息收集完成后（10题问完或信息已足够），必须先 1-2 句话简要确认用户的回答，然后调用一次 ask_question 工具把 generate_plan 设为 true（question、options 留空）结束采集——这是结束对话的唯一方式，后续流程由系统接管
· 若用户中途表示"生成计划书/差不多了/就这些"等，也可直接这样调用工具结束采集
· 严禁在对话中做总结/梳理/计划方向/生活方式建议——这些都属于《个人健康改善计划书》的范畴，对话中不要做
· 严禁在对话中承诺"后续我会帮您/挂在这里"或询问"您看行吗/同意吗"——只要认为信息足够了，按上面要求调用 ask_question 工具（generate_plan=true）即结束，系统会自动生成计划书
...
10. 过去具体试过哪些方法、结果如何【可多选】
· 所有向家长的提问都必须通过调用 ask_question 工具提交：题干填 question、各选项填 options（每项一个字符串、不带编号），多选时把 multi_select 设为 true；不要在正文里输出选项列表或选项提示文字
· 可多选题的最后一个选项保留"以上都不是/不确定"类兜底项（放入 options 中，前端会自动处理与其他选项互斥）
· 提纲信息收集完成后（10题问完或信息已足够），必须先用1-2句话简要确认家长的回答，然后调用一次 ask_question 工具把 generate_plan 设为 true（question、options 留空）结束采集——这是结束对话的唯一方式，后续流程由系统接管
· 若家长中途表示"生成计划书/差不多了/就这些"等，也可直接这样调用工具结束采集
· 严禁在对话中做总结/梳理/计划方向/生活方式建议——这些都属于《个人健康改善计划书》的范畴，对话中不要做
· 严禁在对话中承诺"后续我会帮您/挂在这里"或询问"您看行吗/同意吗"——只要认为信息足够了，按上面要求调用 ask_question 工具（generate_plan=true）即结束，系统会自动生成计划书
...
· 语气亲和、简洁，每次回复尽量不超过200字`;
  ...
  const MODULES = {
    ...
    bloodNutrition: {
      ...
      build: (ctx) => {
        ...
        const rows = lab.indicators
          .map((i) => `${i.name}　${i.value}　${i.flag}　${i.normal}`)
          .join("\n");
  ...
  const PLAN_GENERATE_NATURAL_RE =
    /(?:马上|稍后|现在|接下来|立即|稍等|即将|现在).{0,12}生成.{0,12}计划书/;

  // 各版本"体检核心问题"摘要（供计划书生成使用）
  const CORE_PROBLEMS = {
    male38:
      "以体重增加为中心的一串代谢异常：超重（BMI 27.4、腰围 96cm）、血脂超标（总胆固醇 5.72、甘油三酯 2.38、LDL-C 3.56 均偏高）、轻度脂肪肝伴肝酶升高（ALT 68、GGT 86）、尿酸偏高（468）、血糖临界（HbA1c 5.7%）、血压正常高值（132/86）。甲状腺右叶一枚 6×5mm TI-RADS 3类结节，良性可能大，定期复查即可。",
  };
  ...
  function buildAnswersSummary(messages) {
    ...
    for (const m of messages) {
      ...
      if (m.role === "assistant") {
        const q = questionFromMessage(m);
        if (q) question = q.replace(PLAN_GENERATE_RE, "").trim();
      } else if (m.role === "user") {
  ...
  function buildReportSummary(ctx) {
    ...
    if (lab && lab.indicators && lab.indicators.length) {
      ...
      lab.indicators.forEach((i) =>
        lines.push(i.name + "　" + i.value + "　" + i.flag + "　" + i.normal),
      );
  ...
  function handleGeneratePlan() {
    ...
    try {
      localStorage.setItem("reportPlanCtx", payload);
    } catch (e) {
      /* 忽略 */
    }
    try {
      sessionStorage.setItem("reportPlanCtx", payload);
    } catch (e) {
      /* 忽略 */
    }
    // 采集信息已更新，清除旧计划书缓存，点击卡片时用最新信息重新生成
    try {
      localStorage.removeItem("reportPlan");
    } catch (e) {
      /* 忽略 */
    }
    appendPlanCard();
  ...
  function buildSystemPrompt(ctx, templateOverride) {
    ...
    if (
      ctx.lab &&
      ctx.lab.indicators &&
      ctx.lab.indicators.length &&
      !modules.includes("bloodNutrition")
    ) {
      modules.push("bloodNutrition");
    }
    const template =
      templateOverride ||
      PROMPT_TEMPLATES[ctx.version] ||
    ...
    prompt = prompt
      .split("\n")
      .filter((line) => !/\{MODULE_[A-Z_]+\}/.test(line))
      .join("\n");
    return prompt;
  }
  ...
  function saveHistory(messages, profile) {
    try {
      ...
      if (messages.length > MAX_TAIL + 1) {
        ...
        // 若尾部第一帧是 user（说明半对被截断）或 tool（其配对的 assistant 已丢失），丢弃它，保证从 assistant 开始
        if (tail.length && (tail[0].role === "user" || tail[0].role === "tool"))
          tail = tail.slice(1);
  ...
  // ===== 选择题工具调用定义（结构化获取题干/选项，替代文本解析）=====
  const ASK_QUESTION_TOOL = {
    type: "function",
    function: {
      name: "ask_question",
      description:
        "需要用户从多个答案中选择（选择题）时必须调用此工具，把题干和选项以结构化参数提交，前端会自动渲染为可点击的选项按钮。每次只能调用一次、只能提一个问题。若需要结束对话并生成健康计划书，可将 generate_plan 设为 true 并留空 question/options。",
      parameters: {
        type: "object",
        properties: {
          question: {
            type: "string",
            description: '题干文字（问句）。不要包含"多选/多选题"等标注。',
          },
          options: {
            type: "array",
            items: { type: "string" },
            ...
            maxItems: 8,
            description:
              '选项内容列表，每项一个字符串，不要带序号、不要带"请选择"等提示文字。',
          },
          multi_select: {
            type: "boolean",
            description: "该题是否允许多选，前端据此渲染多选交互。",
          },
          generate_plan: {
            type: "boolean",
            description:
              "信息收集完成、需要结束对话并生成健康计划书时设为 true。",
          },
        },
        required: ["generate_plan"],
      },
    },
  };

  // 解析 assistant 消息中的工具调用（ask_question）为结构化对象；无则返回 null
  function parseQuestionTool(msg) {
    const calls = Array.isArray(msg.tool_calls) ? msg.tool_calls : [];
    const tc = calls.find(
      (c) => c.function && c.function.name === "ask_question",
    );
    if (!tc || !tc.function.arguments) return null;
    try {
  ...
  // 从 AI 消息生成"可视化结构"：正文 + 选项 + 是否多选。
  // 优先读取工具调用返回的结构化数据；无工具调用时回退用 parseOptions 解析正文文本（兼容旧历史）
  function visualFromMessage(msg) {
    const content = typeof msg.content === "string" ? msg.content : "";
    const base = content.replace(PLAN_GENERATE_RE, "").trim();
    const parsed = parseQuestionTool(msg);
    if (parsed && parsed.generate_plan) {
      return { body: base, options: [], multi: false };
    }
    if (parsed && Array.isArray(parsed.options) && parsed.options.length >= 1) {
      ...
      return {
        ...
        multi: !!parsed.multi_select,
      };
    }
    const { body, options } = parseOptions(content);
    return {
      body: (body && body.trim()) || base,
      options,
      multi: isMultiSelect(content),
    };
  }

  // 是否应触发"生成计划书"：正文含标记（兜底）或工具调用 generate_plan=true
  function shouldGeneratePlan(msg) {
    const content = typeof msg.content === "string" ? msg.content : "";
    if (
      PLAN_GENERATE_RE.test(content) ||
      PLAN_GENERATE_NATURAL_RE.test(content)
    )
      return true;
    const parsed = parseQuestionTool(msg);
    return !!(parsed && parsed.generate_plan);
  }

  // 从 AI 消息提取"题干"（供计划书生成时的回答摘要使用）
  function questionFromMessage(msg) {
    const parsed = parseQuestionTool(msg);
    if (parsed && parsed.question) return String(parsed.question);
    const content = typeof msg.content === "string" ? msg.content : "";
    return parseOptions(content).body || "";
  }
  ...
  function parseOptions(text) {
      ...
      if (m && m[3]) {
        ...
        if (count >= 2 && count > best.count) {
          const lookback = lines
            .slice(Math.max(0, curStart - 5), curStart)
            .join("\n");
    ...
    if (/或者|还是/.test(text)) {
      // 拆分"你想了解A，还是B，还是C"
      const m = text.match(
        /^([^，？。\?:：]+?)[，：:]?\s*([^，？。\?:：]+?)\s*或者\s*([^？。\?:：]+?)(?:\s*或者\s*([^？。\?:：]+?))?(?:\s*或者\s*([^？。\?:：]+?))?\??$/,
      );
      if (m) {
        const intro = m[1].trim() + "：";
        const opts = [m[2], m[3], m[4], m[5]]
          .filter(Boolean)
          .map((s) => s.trim());
        if (opts.length >= 2) {
  ...
  // visual = { body, options, multi }：优先级来自工具调用结构化数据，兼容文本解析兜底
  function renderBotReply(typingEl, visual) {
    const body = (visual && visual.body) || "";
    const options = (visual && visual.options) || [];
    const multi = !!(visual && visual.multi) || isMultiSelect(body);
    ...
    if (options.length > 0) {
      ...
      if (multi) {
        ...
        hint.innerHTML =
          '<span class="option-list__hint-mark">多选</span>本题可多选，请选择所有符合的选项';
      ...
      options.forEach((opt, i) => {
        ...
        if (
          /以上都不是|以上都不太像|都不太像|不太像|不确定|都不太好改|没有特别明显/.test(
            opt,
          )
        ) {
        ...
        btn.innerHTML =
          '<span class="option-list__index">' +
          (i + 1) +
          "</span>" +
          '<span class="option-list__text">' +
          escapeHtml(opt) +
          "</span>" +
          (multi
        ...
        btn.addEventListener("click", () => {
          if (multi) {
            if (btn.dataset.fallback) {
              // 选中兜底项：清空其他选中
              list
                .querySelectorAll(".option-list__item.is-selected")
                .forEach((b) => b.classList.remove("is-selected"));
              btn.classList.add("is-selected");
            } else {
              // 选中具体项：取消兜底项
              const fallback = list.querySelector(
                ".option-list__item[data-fallback]",
              );
            ...
            list.querySelectorAll(".option-list__item").forEach((b) => {
              b.disabled = true;
            });
            sendPrompt(opt, true);
          }
      ...
      if (multi) {
        ...
        confirm.addEventListener("click", () => {
          const selected = Array.from(
            list.querySelectorAll(".option-list__item.is-selected"),
          ).map(
            (b) =>
              b.dataset.optIndex +
              "、" +
              b.querySelector(".option-list__text").textContent,
          );
          if (!selected.length) return;
          list
            .querySelectorAll(".option-list__item, .option-list__confirm")
            .forEach((b) => {
              b.disabled = true;
            });
  ...
  function renderHistoryMessage(msg) {
    if (!msg || msg.role === "system" || msg.role === "tool") return;
    const content = typeof msg.content === "string" ? msg.content : "";
    if (msg.role === "user") {
      appendMessage(content, true);
    } else if (msg.role === "assistant") {
      const visual = visualFromMessage(msg);
      const text = visual.body || content;
      const item = document.createElement("div");
      item.className = "chat__item chat__item--bot";
      item.innerHTML = `
        <div class="chat__avatar">
  ...
  // 规整对话上下文：移除悬空的 tool 响应 / 无配对 tool 响应的 tool_calls，避免聊天协议被破坏
  ...
  // 流式 SSE 读取（OpenAI 兼容接口，stream:true）
  // 解析 `data:` 帧：content 增量与 tool_calls 增量分开累计；onDelta 收到单帧 delta。
  // 返回 { content, toolCalls }，与一次性 json 读取的结构等价；代理不支持流时自动退化。
  async function readSSE(res, onDelta) {
    const buildToolCalls = (map) =>
    ...
    const fallback = async () => {
      ...
      const msg =
        data.choices && data.choices[0] ? data.choices[0].message : null;
      const content =
        msg && typeof msg.content === "string" ? msg.content : "";
      const toolCalls =
        msg && Array.isArray(msg.tool_calls) ? msg.tool_calls : [];
      if (onDelta && (content || toolCalls.length))
        onDelta({ content: content, tool_calls: toolCalls });
      return { content, toolCalls };
    };
    if (!(res.body && res.body.getReader)) return fallback();
    ...
    const done = () => ({
      ...
      toolCalls: buildToolCalls(toolMap),
    });
    ...
    for (;;) {
      ...
      while ((idx = buf.indexOf("\n")) !== -1) {
        ...
        if (Array.isArray(delta.tool_calls)) {
          ...
          if (onDelta) onDelta({ tool_calls: delta.tool_calls });
        }
      }
    }
    return done();
  ...
  async function sendPrompt(content, showUser = true) {
    ...
    try {
      ...
      // 流式输出：正文边生成边渲染到气泡；工具调用分片同步累计，结束后复用原有结构化渲染
      let acc = "";
      const streamed = await readSSE(res, (delta) => {
        if (delta && typeof delta.content === "string" && delta.content) {
          acc += delta.content;
          typing.textContent =
            acc.replace(PLAN_GENERATE_RE, "").trim() || "正在思考…";
          scrollToBottom();
        }
      });
      const rawContent = streamed.content;
      const toolCalls = streamed.toolCalls;
      if (!rawContent && !toolCalls.length) {
        throw new Error("暂时没有收到有效回复，请稍后再试。");
      }

      // 保留原始 assistant 回复（含工具调用）+ 补一条 tool 结果，保证对话协议完整、后续轮次可继续
      ...
      // 结构化的可视化内容：优先工具调用数据（题干/选项/多选），回退文本解析
      const visual = visualFromMessage(assistantMsg);
      renderBotReply(typing, visual);
      scrollToBottom();
      // 成功后持久化对话历史
      if (currentCtx) saveHistory(messages, currentCtx.version);
      // 检测采集完成 → 整理回答并跳转计划书页（工具 generate_plan=true 或正文含标记均触发）
      if (shouldGeneratePlan(assistantMsg)) {
  ...
  function showGreeting(text, introUser, options) {
    ...
    if (currentCtx) {
      try {
        saveHistory(messages, currentCtx.version);
      } catch (e) {
        /* 忽略 */
      }
    }
    ...
    if (options && options.length) {
      ...
      options.forEach((opt, i) => {
        ...
        btn.innerHTML =
          ...
          '<span class="option-list__arrow">›</span>';
        btn.addEventListener("click", () => {
          wrap.querySelectorAll(".option-list__item").forEach((b) => {
            b.disabled = true;
          });
  ...
  const PLAN_BUILD_INTENT_RE =
    /(?:制定|生成|制作|定制)(?:个人|专属|健康|改善)?计划(?:书)?|做(?:个|份|一下).{0,6}计划|健康(?:改善)?计划|改善计划/;
  // 计划书流程中，用户表达暂停/退出/换话题 → 退出计划书流程
  const PLAN_BUILD_EXIT_RE =
    /暂停|退出|不做了|先(?:聊|问|说|做)别的|算了|换个话题|不制定|不做计划|停一下|别问了|下次再说/;
  ...
  function startPlanBuild(userText) {
    const ctx = currentCtx || { disease: "健康问题", profile: {}, lab: null };
    const isChild =
      /^child/i.test(ctx.version || "") ||
      /儿童|男童|女童|孩子/.test(ctx.disease || "");
  ...
  async function init() {
    ...
    try {
      const raw =
        localStorage.getItem("reportCtx") ||
        sessionStorage.getItem("reportCtx");
