(() => {
  // ===== AI 接口配置（统一在项目根目录 llm-config.js，勿在此重复配置）=====
  ...
  function renderPlan(text) {
    ...
    for (let i = 1; i < parts.length; i += 3) {
      ...
      let num = aiNum;
      if (aiIdx >= 4)
        num = String((parseInt(aiNum, 10) || 0) + 1).padStart(2, "0");
      toc.push({ num, title });
      const tinted = isFollowup;
      // 随访计划：按时间节点分块渲染，并注入导流组件
      const bodyHtml = isFollowup
        ? renderFollowupSection(body)
      ...
      html +=
        ...
        num +
        '">' +
        '<div class="section-head"><p class="section-eyebrow">' +
        num +
  ...
  function renderSectionBody(body) {
    ...
    while (i < blocks.length) {
        ...
        html +=
          ...
          goalBlocks
            .map((gb, idx) =>
              renderGoalCard(
                String(idx + 1),
                gb.title.replace(/^目标\s*\d+\s*[｜|]\s*/, "").trim(),
                gb.lines.join("\n"),
              ),
            )
            .join("") +
      ...
      // 关键词 chips 块
      const kwM = b.title.match(
        /^(?:当前3个主要攻克点|当前3个关键词|你的\s*3个关键词|你的3个关键词)$/,
      );
      if (kwM) {
        const chipTexts = [];
        ...
        if (chipTexts.length)
          html +=
            '<div class="chips">' +
            chipTexts
              .map((c) => '<span class="chip">' + inlineMd(c) + "</span>")
              .join("") +
            "</div>";
        i++;
        continue;
      ...
      if (modM) {
        const innerTitles =
          /你现在的问题是什么|接下来具体怎么做|做什么运动|做到什么强度|每次多久|每周几次|每周\/每日|30天|本阶段目标|你要做的|完成标准|没做到|怎么测/;
        let j = i + 1;
        ...
        while (j < blocks.length) {
          ...
          const isInner =
            nb.title &&
            innerTitles.test(nb.title) &&
            !/^[A-Ha-h]\s*[.、]\s/.test(nb.title);
          if (!isInner) break;
      ...
      // 普通子标题块
      if (b.title)
        html += '<h3 class="sub-title">' + escapeHtml(b.title) + "</h3>";
  ...
  function renderFollowupSection(body) {
    ...
    const BELL_SVG =
      '<span class="fu-check__icon" aria-hidden="true"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 9-3 9h18s-3-2-3-9"></path><path d="M13.73 21a2 2 0 0 1-3.46 0"></path></svg></span>';
    ...
    for (const b of blocks) {
      ...
      let label = t;
      const tm = t.match(
        /^((?:每周|每日|每周\/每日|日常数据记录|[\d一二三四五六七八九十]+\s*个月?后|[\d]+\s*个月后)[^：:]*)[·・\s]*(.*)$/,
      );
      if (tm) {
        tag = tm[1].trim();
      ...
      html +=
        ...
        (tag
          ? '<span class="followup-tag">' + escapeHtml(tag) + "</span>"
          : "") +
        escapeHtml(label) +
        "</h3>";
      html += renderProse(b.lines.join("\n"));
      ...
      if (isDaily) {
        html +=
          '<button type="button" class="fu-btn fu-btn--outline" data-fu-device>从设备导入数据</button>';
      } else if (isQ1) {
        html +=
          '<label class="fu-check"><span class="fu-check__text">1 个月后提醒我线上随访</span>' +
          BELL_SVG +
          '<span class="fu-check__box"></span><input type="checkbox" data-fu-remind /></label>';
      } else if (isQ3 || isQ6) {
        html +=
          '<button type="button" class="fu-btn" data-fu-appt>预约线下复查</button>';
      }
      html += "</div>";
    }
    return html;
  ...
  function renderLaunchSection(toc) {
    ...
    return (
      '<section class="section section--launch" id="s04">' +
  ...
  function renderActionLauncher() {
    ...
    const checkRow = (key) =>
      ...
      '<span class="hm-check"><input type="checkbox" checked data-hm-item="' +
      key +
      '" /><span class="hm-check__box"></span></span>' +
  ...
  function bindFollowupActions() {
    ...
    scope.querySelectorAll("[data-hm-confirm]").forEach((b) => {
      b.addEventListener("click", () => {
        ...
        showToast(
          names.length
            ? "已为您开启" + names.join("、") + "健康辅助"
            : "未选择任何健康辅助功能，可随时在后续调整",
        );
      });
    });
  }
  ...
  function renderGoalCard(num, title, body) {
    ...
    const headIcon =
      '<svg class="goal-icon" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg>';
    let quantHtml = "";
    let rowHtml = "";
    for (const f of fields) {
      if (/天目标|月目标/.test(f.label)) {
        quantHtml =
          '<div class="goal-quant"><p class="goal-quant-label">' +
          escapeHtml(f.label) +
          '</p><p class="goal-quant-value">' +
          inlineMd(f.value) +
          "</p></div>";
      } else {
        rowHtml +=
          '<div class="goal-row"><p class="goal-row-label">' +
          escapeHtml(f.label) +
          '</p><p class="goal-row-value">' +
          inlineMd(f.value) +
          "</p></div>";
      }
    }
    return (
      '<article class="goal-card"><div class="goal-head">' +
      headIcon +
      '<div><p class="goal-tag">目标 ' +
      num +
      '</p><h3 class="goal-title">' +
      escapeHtml(title) +
      "</h3></div></div>" +
      '<div class="goal-body">' +
      quantHtml +
  ...
  function renderModule(letter, title, body) {
    let html =
      '<div class="module-head"><span class="module-letter">' +
      letter +
      '</span><h3 class="module-title">' +
      escapeHtml(title) +
      "</h3></div>";
    ...
    for (let i = 1; i < blocks.length; i += 2) {
      ...
      if (/你现在的问题是什么/.test(h)) {
        html +=
          '<p class="section-caption">你现在的问题是什么</p>' + renderProse(hb);
      } else if (/接下来具体怎么做/.test(h)) {
        html +=
          '<p class="section-caption">接下来具体怎么做</p>' + renderActions(hb);
      } else {
  ...
  function renderActions(text) {
    ...
    for (const raw of lines) {
      ...
      const planb = line.match(
        /^[-*]\s*\**\s*【?\s*Plan\s*B\s*】?\s*\**\s*[：:]\s*(.+)$/i,
      );
      if (planb) {
    ...
    for (const c of cards) {
      ...
      if (c.title)
        html += '<h4 class="action-title">' + inlineMd(c.title) + "</h4>";
      if (c.body) html += '<p class="action-text">' + inlineMd(c.body) + "</p>";
      if (c.planb)
        html +=
          '<p class="planb"><strong>Plan B</strong> ' +
          inlineMd(c.planb) +
          "</p>";
      html += "</div>";
    }
  ...
  function renderPhase(num, title, body) {
    ...
    for (const raw of body.split("\n")) {
      ...
      if (fm) {
        ...
        } else if (/本阶段目标/.test(lb)) {
          mode = "goal";
          fields.goal = val;
        } else if (/完成标准/.test(lb)) {
          mode = "done";
          fields.done = val;
        } else if (/没做到|没达成/.test(lb)) {
          mode = "miss";
          fields.miss = val;
        } else mode = "";
        continue;
      ...
      if (ol) {
        todoList.push(ol[1].trim());
        continue;
      }
      const ul = line.match(/^[-*]\s+(.+)$/);
      if (ul) {
        if (mode === "todo") todoList.push(ul[1].trim());
        continue;
      }
      if (mode === "goal")
        fields.goal = (fields.goal ? fields.goal + " " : "") + line;
    }

    let html =
      '<div class="timeline-item"><span class="timeline-num">' +
      num +
      '</span><div class="phase-card">';
    html +=
      '<div class="phase-head"><h3 class="phase-title">第 ' +
      num +
      " 阶段｜" +
      escapeHtml(label) +
      "</h3>" +
      (days ? '<span class="phase-days">' + escapeHtml(days) + "</span>" : "") +
      "</div>";
    if (fields.goal)
      html +=
        '<p class="phase-goal"><strong>本阶段目标：</strong>' +
        inlineMd(fields.goal) +
        "</p>";
    if (todoList.length) {
      html +=
        '<p class="phase-list-label">你要做的 3 件事</p><ul class="phase-list">';
      todoList.forEach((t, idx) => {
        html +=
          '<li><span class="phase-num">' +
          (idx + 1) +
          '</span><span class="phase-list-item">' +
          inlineMd(t) +
          "</span></li>";
      });
      html += "</ul>";
    ...
    if (fields.done || fields.miss) {
      ...
      if (fields.done)
        html +=
          '<p class="phase-complete"><strong>完成标准：</strong>' +
          inlineMd(fields.done) +
          "</p>";
      if (fields.miss)
        html +=
          '<p class="phase-miss"><strong>没做到怎么办：</strong>' +
          inlineMd(fields.miss) +
          "</p>";
      html += "</div>";
    }
    html += "</div></div>";
    ...
    while (i < lines.length) {
      ...
      if (line.startsWith(">")) {
        ...
        html +=
          ...
          quoteLines.map((q) => "<p>" + inlineMd(q) + "</p>").join("") +
          "</blockquote>";
        continue;
      }
      // 关键词 chips（可跳过 ### 与列表间的空行）
      const kw = line.match(
        /^###\s*((?:当前3个主要攻克点|当前3个关键词|你的\s*3个关键词|你的3个关键词))\s*$/,
      );
      if (kw) {
        closeList();
        flushChips();
      ...
      if (h3) {
        closeList();
        flushChips();
        html += '<h4 class="h4">' + inlineMd(h3[1]) + "</h4>";
        i++;
        continue;
      }
      // 无序列表
  ...
  function buildToc(toc) {
    ...
    toc.forEach((t) => {
      ...
      a.href = "#s" + t.num;
      a.innerHTML =
        '<span class="toc-link-inner"><span class="toc-num">' +
        t.num +
  ...
  function fillMeta() {
    ...
    if (!metaStore.owner && o) {
      try {
        const raw =
          localStorage.getItem("reportPlanCtx") ||
          sessionStorage.getItem("reportPlanCtx");
        if (raw) {
          const c = JSON.parse(raw);
          const m = String(c.reportSummary || "").match(
            /姓名[：:]\s*([^\s，,。、]+)/,
          );
  ...
  // 逐行解析 `data:` 帧，把增量 delta 交给 onDelta；返回完整 content 文本。
  // 代理不支持流（res.body 不可读）时退化为一次性 json 读取。
    ...
    if (!(res.body && res.body.getReader)) {
      ...
      const text =
        data.choices && data.choices[0] && data.choices[0].message
          ...
          : "";
      if (text && onDelta) onDelta({ content: text });
      return text;
    }
  ...
  async function generate() {
    ...
    const planStream = document.getElementById("planStream");
    if (planStream) {
      planStream.textContent = "";
      planStream.hidden = true;
    }

    if (!raw) {
      showLoadingState(
        "未找到您的信息采集记录，请回到对话完成信息收集后再生成。",
        "",
      );
      return;
    }
    ...
    const planPrompt = isChild
      ? window.PLAN_PROMPT_TEMPLATE_CHILD
      : window.PLAN_PROMPT_TEMPLATE;
    if (!planPrompt) {
    ...
    const messages = [
      { role: "system", content: system },
      {
        role: "user",
        content: isChild
          ? "请根据以上信息，为我的孩子制定一份专属的《个人健康改善计划书-第一个月》。"
          : "请根据以上信息，为我制定专属的《个人健康改善计划书-第一个月》。",
      },
    ];

    planContent.innerHTML = "";
    showLoadingState(
      "正在结合您的体检结果与回答，生成专属计划书…",
      "生成内容较长，约需 30–60 秒，请耐心等待",
    );
    scrollToTop();
    ...
    try {
      ...
      const fillPlaceholders = (t) =>
        t
          .replace(/\{\{PLAN_START\}\}/g, fmt(now))
          .replace(/\{\{PLAN_END\}\}/g, fmt(end))
          .replace(/\{\{[A-Z_]+\}\}/g, "");

      // 流式输出：loading 区实时展示生成进度预览，完整内容收到后再做结构化渲染
      const planStream = document.getElementById("planStream");
      let acc = "";
      let streamRaf = 0;
      const content = await readSSE(res, (delta) => {
        if (!delta || typeof delta.content !== "string") return;
        acc += delta.content;
        if (!planStream) return;
        if (!streamRaf) {
          streamRaf = requestAnimationFrame(() => {
            streamRaf = 0;
            planStream.textContent = fillPlaceholders(acc);
            planStream.hidden = !planStream.textContent;
          });
        }
      });
      if (streamRaf) cancelAnimationFrame(streamRaf);
      if (!content) throw new Error("未收到有效回复，请重试");
      const finalContent = fillPlaceholders(content);
      ...
      try {
        const cachedCtx = (() => {
          try {
            return JSON.parse(localStorage.getItem("reportPlanCtx"));
          } catch (e) {
            return null;
          }
        })();
      ...
      const offlineVer = (() => {
        try {
          return (
            (JSON.parse(localStorage.getItem("reportPlanCtx")) || {}).version ||
            ""
          );
        } catch (e) {
          return "";
  ...
  (function initPlan() {
    ...
    if (
      ...
      (!cached.version || !currentVersion || cached.version === currentVersion)
    ) {
