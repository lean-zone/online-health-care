...
· 结尾固定附一句：本小结为健康管理建议，最终以医生面诊与检查安排为准`;
  ...
  function buildSystemPrompt(ctx) {
    ...
    if (ctx && ctx.history)
      known.push("既往体检异常 / 慢性病史：" + ctx.history);
    if (ctx && ctx.symptom)
      known.push("用户主动提出的健康关注点：" + ctx.symptom);
  ...
  function buildSummaryMessages(messages) {
    ...
    return [
      ...
      },
      {
        role: "user",
        content:
          "以下是本次体检选购咨询的完整对话记录，请据此生成《体检方案推荐小结》：\n\n" +
          transcript,
      },
    ];
  }
  ...
  const KB_MISSING_TIP =
    "（知识库本次未加载成功：不要编造具体套餐名与价格，只给方向性建议，并提示以卓正官方渠道为准。）";
  ...
  function parseKbSections(text) {
    ...
    return parts.map((chunk) => {
      ...
      return {
        title: title,
        body: (idx === -1 ? "" : chunk.slice(idx + 1)).trim(),
      };
    });
  }
  ...
  function detectExamTarget() {
    ...
    if (/孩子|儿童|儿子|女儿|小朋友|青少年|宝宝|入园|入学/.test(first))
      return "child";
  ...
  function knowledgeBlock() {
    ...
    const isChild = target
      ? target === "child"
      : /孩子|儿童|儿子|女儿|小朋友|青少年|宝宝|入园|入学/.test(text);
    const isFemale = target
      ? target === "female"
      : /女性|妈妈|母亲|妻子|老婆|乳腺|妇科|宫颈|HPV|孕/.test(text);
    const isMale = target
      ? target === "male"
      : /男性|爸爸|父亲|老公|丈夫|前列腺|泌尿/.test(text);
    ...
    if (isChild) {
      ...
      if (isFemale && isMale) {
        const compare = kbFind("横向对比");
        if (compare) picked.push(compare);
      }
  ...
  function stageLabel() {
    if (currentPhase === "S7")
      return "S7｜最终方案推荐（输出 1 张主套餐卡 + 加项卡 + 推荐理由 + 「这个方案可以吗？」选项）";
    if (currentPhase === "S6")
      return "S6｜精准追问中（仅出选择题，禁止输出【套餐卡】与推荐理由）";
    if (currentPhase === "S5")
      return 'S5｜初步推荐已出（输出两张【套餐卡】+推荐理由，不再列选项，等用户点"继续提问"或继续提问）';
  ...
  function summaryFlagKey(profile) {
    return storageKey(profile) + "_summary";
  }
  function setSummaryFlag(profile) {
    try {
      localStorage.setItem(summaryFlagKey(profile), "1");
    } catch (e) {
      /* 存储不可用时忽略 */
    }
  }
  function readSummaryFlag(profile) {
    try {
      return localStorage.getItem(summaryFlagKey(profile)) === "1";
    } catch (e) {
      return false;
    }
  }
  function clearSummaryFlag(profile) {
    try {
      localStorage.removeItem(summaryFlagKey(profile));
    } catch (e) {
      /* 存储不可用时忽略 */
    }
  }
      ...
      if (t !== null) {
        ...
        if (count >= 2 && count > best.count) {
          const lookback = lines
            .slice(Math.max(0, curStart - 8), curStart)
            .join("\n");
          best = {
            start: curStart,
            end: idx - 1,
            count,
            hasQuestion: /[?？]/.test(lookback),
          };
        }
        curStart = -1;
        count = 0;
  ...
  function attachFreeInput(list, multi, lock) {
    ...
    const submit = () => {
      ...
      if (multi) {
        ...
        if (
          max &&
          list.querySelectorAll(".option-list__item.is-selected").length >= max
        ) {
        ...
        chip.innerHTML =
          '<span class="option-list__index">+</span>' +
  ...
  function splitCardSegments(text) {
    ...
    while ((m = markerRe.exec(text)) !== null) {
      ...
      const endAbs =
        m.index + m[0].length + endMatch.index + endMatch[0].length;
  ...
  function pkgDetailUrl(name) {
    ...
    return (
      "pkg-detail.html?g=female&t=" + t + "&gyn=" + (/含妇科/.test(n) ? 1 : 0)
    );
  }
  ...
  function buildCardEl(card) {
    ...
    let html =
      '<span class="pkg__head">' +
      '<span class="pkg__tag">' +
      escapeHtml(card.tier || "推荐") +
      "</span>" +
      '<span class="pkg__name">' +
      escapeHtml(card.name || "套餐") +
      "</span>" +
      "</span>";
    html +=
    ...
    if (card.diff) {
      ...
      html +=
        '<span class="pkg__row pkg__row--diff"><span class="pkg__k">' +
        escapeHtml(label) +
        '</span><span class="pkg__v">' +
        escapeHtml(card.diff) +
        "</span></span>";
    }
    // 注意事项（如空腹、经期不做）：可选的补充说明，卡内以浅黄标签呈现
    if (card.note) {
      html +=
        '<span class="pkg__row pkg__row--note"><span class="pkg__k">注意</span><span class="pkg__v">' +
    ...
    if (hasDetail) {
      ...
      html +=
        ...
        (isChild ? "儿童独立方案 · 详情请咨询顾问" : "点击查看套餐详情 ›") +
        "</span>";
    }
    el.innerHTML = html;
  ...
  function buildReasonEl(reason, title) {
    ...
    el.innerHTML =
      ...
      '<p class="pkg__reason__title">' +
      escapeHtml(title || "为什么这样推荐") +
      "</p>" +
  ...
  function renderOptions(bubble, body, options, multi, readonly) {
    ...
    list.className =
      "option-list" +
      (multi ? " option-list--multi" : "") +
      (readonly ? " option-list--readonly" : "");
    ...
    const lockList = () => {
      ...
      if (host)
        host.querySelectorAll(".option-list__skip").forEach((b) => {
          b.disabled = true;
        });
    };
    ...
    let maxSelect = 0;
    if (multi && !readonly) {
      const maxMatch = (body || "").match(
        /最多(?:可以)?(?:可选|选择|选|择)?\s*(\d+)\s*项/,
      );
      maxSelect = maxMatch
        ? Math.max(1, Math.min(8, parseInt(maxMatch[1], 10) || 0))
        : 0;
      if (maxSelect) list.dataset.max = String(maxSelect);
      const hint = document.createElement("div");
      hint.className = "option-list__hint";
      hint.innerHTML =
        '<span class="option-list__hint-mark">多选</span>' +
        (maxSelect
          ? "本题最多可选 " + maxSelect + " 项"
          : "本题可多选，请选择所有符合的选项");
    ...
    options.forEach((opt, i) => {
      ...
      if (
        /以上都不是|以上都不太像|都不太像|不太像|不确定|没有特别明显/.test(opt)
      ) {
      ...
      btn.addEventListener("click", () => {
        if (multi) {
          const picked = list.querySelectorAll(
            ".option-list__item.is-selected",
          ).length;
          if (
  ...
  function appendReasonAndQuestion(
    bubble,
    reasonHtml,
    fallbackReason,
    pendingQuestion,
  ...
  function renderBotReply(typingEl, reply) {
    ...
    appendReasonAndQuestion(
      bubble,
      segs.reasonHtml,
      reasonText,
      segs.pendingQuestion,
    );
    ...
    if (options.length === 0 && body && /[?？]/.test(body)) {
      ...
      btn.addEventListener("click", () => {
        btn.disabled = true;
        sendPrompt(
          "上一轮没收到选项，请把刚才的问题重新整理一次：先单独一行写题干（以 ？结尾），再单独一行写「选项：」，下面逐行列出 2-6 个可点击的选项。",
          true,
        );
      });
  ...
  function renderHistoryMessage(msg, interactive) {
    ...
    const content =
      typeof msg.content === "string" ? stripStageTag(msg.content) : "";
    if (msg.role === "user") {
  ...
  // onDelta 用于流式进度（每次收到内容增量），返回完整 content
  async function callAI(payloadMessages, timeoutMs = REQ_TIMEOUT_MS, onDelta) {
    const controller = new AbortController();
    ...
    try {
      ...
      const content = await readSSE(res, onDelta);
  ...
  function setInputLocked(locked) {
    ...
    document.querySelectorAll(".footer-actions .chip").forEach((b) => {
      b.disabled = locked;
    });
    updateSendButton();
  }
  ...
  const OPTION_LOCK_SELECTOR =
    ".option-list__item, .option-list__confirm, .option-list__skip, " +
  ...
  function setOptionsLocked(locked, nodes) {
    ...
    Array.prototype.forEach.call(
      chatList.querySelectorAll(OPTION_LOCK_SELECTOR),
      (b) => {
        if (b && typeof b.disabled === "boolean" && !b.disabled) {
          b.disabled = true;
          enabled.push(b);
        }
      },
  ...
  async function sendPrompt(content, showUser = true) {
    ...
    try {
      ...
      // 流式输出：正文边生成边渲染到气泡，完整内容收到后再解析选项/卡片
      let acc = "";
      const rawReply = await callAI(buildApiMessages(), REQ_TIMEOUT_MS, (delta) => {
        if (!delta || typeof delta.content !== "string") return;
        acc += delta.content;
        typing.textContent =
          acc.replace(SUMMARY_GENERATE_RE, "").trim() || "正在思考…";
        scrollToBottom();
      });

      // 剥离阶段标记与小结标记（都不显示给用户）；阶段先推进，再渲染，保证渲染时拿到的是本轮阶段
      const reply =
        stripStageTag(rawReply).replace(SUMMARY_GENERATE_RE, "").trim() ||
        "抱歉，暂时没有收到有效回复，请稍后再试。";
  ...
  async function openSummarySheet() {
    ...
    renderSheetLoading();
    try {
// 流式输出：小结内容边生成边渲染，避免弹层长时间空白
    let acc = "";
    let rafId = 0;
    const text = await callAI(
      buildSummaryMessages(buildApiMessages(SUMMARY_CONTEXT_TAIL)),
      REQ_TIMEOUT_MS,
      (delta) => {
        if (!delta || typeof delta.content !== "string") return;
        acc += delta.content;
        if (!rafId) {
          rafId = requestAnimationFrame(() => {
            rafId = 0;
            summaryBody.innerHTML = acc ? mdToHtml(acc) : "";
          });
        }
      },
    );
    ...
    summaryBody.innerHTML = summaryCache;
      ...
      summaryBody.innerHTML =
        "<p>小结生成失败：" +
        escapeHtml(e && e.message ? e.message : "未知错误") +
        "。请稍后重试。</p>";
    } finally {
  ...
  document
    .getElementById("summaryClose")
    .addEventListener("click", closeSummarySheet);
  ...
  function showGreeting(text, introUser, options) {
    ...
    messages.push({
      role: "assistant",
      content: withOptionsBlock(text, options),
    });
    scrollToBottom();
    if (currentCtx) {
      try {
  ...
  function readEntryCtx() {
    ...
    try {
      const raw =
        localStorage.getItem("consultCtx") ||
        sessionStorage.getItem("consultCtx");
  ...
  async function init() {
    ...
    if (Array.isArray(history) && history.length > 0) {
      ...
      restored.forEach((m, i) => {
        ...
        try {
          renderHistoryMessage(m, interactive);
        } catch (e) {
          console.warn("历史消息渲染失败：", e);
        }
      });
    ...
    btn.addEventListener("click", () => {
      ...
      if (action === "packages") {
        ...
        window.location.href =
          "pkg-detail.html?g=" + g + "&t=1&gyn=" + (g === "female" ? "1" : "0");
        return;
      }
      // 提问型入口：直接以用户身份发起
  ...
  reportInput.addEventListener("change", () => {
    ...
    sendPrompt(
      "我上传了外院体检报告：" +
        files.join("、") +
        "。请结合这份报告，帮我看看这次体检需要重点复查或补充加项的地方。",
      true,
    );
  });
  ...
  function restartConversation() {
    try {
      localStorage.removeItem(
        storageKey(currentCtx ? currentCtx.version : "guest"),
      );
    } catch (e) {
      /* 忽略 */
    }
