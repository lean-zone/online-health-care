(() => {
  ...
  function buildSystemPrompt(ctx) {
    ...
    const scene = sc.prompt
      .map(function (l) {
        return fillTpl(l, data);
      })
      .filter(Boolean);
    if (scene.length)
      blocks.push("【本次进入场景】" + sc.name + "\n" + scene.join("\n"));

    const p = c.profile || {};
    const who = [
      String(p.name || "").trim(),
      String(p.gender || "").trim(),
      p.age ? String(p.age).trim() + "岁" : "",
    ]
      .filter(Boolean)
      .join("·");
    ...
    return [
      {
        role: "system",
        content:
          "你是卓正医疗减重服务的健康顾问，请根据对话记录生成一份简洁的《减重咨询小结》。",
      },
  ...
  function renderBotReply(typingEl, reply) {
    ...
    if (options.length > 0) {
      ...
      options.forEach((opt, i) => {
        ...
        if (
          /以上都不是|以上都不太像|都不太像|不太像|不确定|没有特别明显/.test(
            opt,
          )
        ) {
  ...
  async function sendPrompt(content, showUser = true) {
    ...
    try {
      ...
      // 流式输出：正文边生成边替换气泡文字，完整内容收到后再做解析/渲染
      let acc = "";
      const rawReply = await readSSE(res, (delta) => {
      ...
      const reply =
        rawReply.replace(SUMMARY_GENERATE_RE, "").trim() ||
        "抱歉，暂时没有收到有效回复，请稍后再试。";
  ...
  async function openSummarySheet() {
    ...
    try {
      ...
      let acc = "";
      let rafId = 0;
      const text = await readSSE(res, (delta) => {
        if (!delta || typeof delta.content !== "string") return;
        acc += delta.content;
        if (!rafId) {
      ...
      summaryBody.innerHTML = text
        ? mdToHtml(text)
        : "<p>暂时没有生成小结内容，请稍后重试。</p>";
    } catch (e) {
  ...
  function readEntryCtx() {
    ...
    out.version =
      valueOf(stored, "version") || valueOf(fromUrl, "version") || "guest";
